...
function App() {
  const [ContentTask, setContentTask] = useState(['Oi'])

  const [newCommentText, setNewCommentText] = useState('')

  function createNewTask(event: FormEvent){
    event.preventDefault() //onSubmit se torna "prefixado", semelhante ao onClick ou type="button"
    setContentTask([...ContentTask, newCommentText])//...comments = herda os comentários dentro de useState([])
    setNewCommentText('')
  }

  function addContentTask( event: ChangeEvent<HTMLInputElement>){
    event.target.setCustomValidity("")
    setNewCommentText(event.target.value)
  }
  const inputIsEmpty = newCommentText == ""
  const ContentTaskIsNotEmpty :boolean = ContentTask != []
  // const ContentTask = [
  //   {
    ...
    <div className="App">
      ...
      <main>
        <form onSubmit={createNewTask}>
          <input value={newCommentText} onChange={addContentTask} type="text" placeholder='Adicione uma nova tarefa' name="" maxLength={150} />
          <button disabled={inputIsEmpty}>
            <span>Criar</span>
        ...
        <section>
          <header>
            <div className="createdTasks">
              <h3>Tarefas criadas</h3> <p>{ContentTask.length}</p>
            </div>
            <div className="finishedTasks">
              <h3>Concluídas</h3> <p>{0} de {ContentTask.length}</p>
            </div>

          </header>
          <div className="tasksList">
            {
              ContentTask.map( post => {
                return(
                  <Task key={post} description={post}/>
                )
              })
            }
            <fieldset disabled={ContentTaskIsNotEmpty} className="contentTaskList">
              <img src={Clipboard} alt="" />
              <div className="textContentTaskList">
